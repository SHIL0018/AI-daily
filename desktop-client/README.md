...
客户端启动后会先检查登录状态。首次使用必须连接服务端登录；本机成功登录并注册过设备后，服务器临时不可用时可以选择离线进入，活动会继续保存在本地。

主界面使用聚焦记录控制台：

- 中央主按钮会根据状态自动切换为“开始记录”“暂停记录”或“恢复记录”。
- 记录中和暂停时会显示次级“停止记录”操作。
- 右侧集中显示记录器、本地模型和同步队列状态。
- 断网或服务器临时故障不会消耗记录的永久重试次数；存在失败记录时，同步按钮会切换为“重试失败记录”。
- 首页只显示最近 3 条活动，“查看全部”会打开本地记录抽屉。
- 右上角齿轮打开设置抽屉；记录运行或暂停时禁止修改设置。
- “打开 Web 日报”保留在顶部栏。

开始记录后，“平均推理”会统计当前记录会话中实际调用本地模型的平均耗时；截图相似而跳过模型的采集不会计入该平均值。
