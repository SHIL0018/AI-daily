...
export function RuntimeStatusPanel({ status, offline, pendingAction, onModelCheck, onSync, onRetryFailed }: {
  ...
  return (
    <aside className="runtime-panel">
      ...
      <div className="runtime-row">
        ...
        <button className="row-action" type="button" title={status?.sync.failed ? "重试失败记录" : "立即同步"} disabled={offline || Boolean(pendingAction)} onClick={status?.sync.failed ? onRetryFailed : onSync}>
          {status?.sync.failed ? <RotateCcw size={16} className={pendingAction === "retry-sync" ? "spinning" : ""} /> : <RotateCw size={16} className={pendingAction === "sync" ? "spinning" : ""} />}
        </button>
      </div>
